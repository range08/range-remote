...
The MCP bridge reads Codex-style `mcp_servers` configuration from `/etc/codex/config.toml`, `$CODEX_HOME/config.toml` (default `~/.codex/config.toml`), and project `.codex/config.toml` files from repository root to the requested working directory. Later layers override earlier ones. It supports STDIO and Streamable HTTP servers, environment-variable forwarding, configured HTTP headers, bearer-token environment variables, tool allow/deny filters, and startup/tool timeouts. Secret environment/header values are consumed only on the paired device and are not returned by `list_mcp_servers`.
...
Interactive downstream OAuth and Codex's `http_headers_helper` are not bridged yet. Servers using those features need a locally available bearer token/header configuration or will return an explicit unsupported/authentication error.
